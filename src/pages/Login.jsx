...
export const Login = () => {
  const { user, validateUser, handleUser } = useContext(ERPContext)
  ...
  return (
    <main className='w-ful h-[90vh] flex justify-center items-center font-text'>
      <Formik
        initialValues={
        {
          name: '',
          ...
          <Form className='flex flex-col justify-evenly items-center max-w-lg w-[300px] h-[450px] px-6 border-[1px] border-[#cccbcb] shadow-lg rounded-[10px]'>
            ...
            <div className='flex flex-col justify-center items-center w-full gap-[15px]'>
              ...
              <div className='flex flex-col justify-between items-center h-[100px] mt-3'>
                <button disabled={!(isValid && dirty)} className='transition ease-in-out w-[120px] h-[40px] bg-[#0495a8] text-white text-[17px] rounded-full mt-[10px] disabled:bg-[#75c0ca]' type='submit'>Ingresar</button>
                {validateUser.isValidating &&
                  <div role='status'>
                ...
                {validateUser.code === 0 && <p className='text-red-500'>User not found</p>}
                {validateUser.code === 1 && <p className='text-red-500'>Password is not valid</p>}
              </div>
            </div>
          </Form>
        )}
